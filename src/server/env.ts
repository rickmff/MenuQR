...
const schema = z.object({
  DATABASE_URL: z.preprocess((value) => (value === '' ? undefined : value), z.string().min(1).default('file:./data/menuqr.db')),
  ...
  NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY: optional(z.string().startsWith('pk_', 'começa com pk_')),
  CLERK_SECRET_KEY: optional(z.string().startsWith('sk_', 'começa com sk_')),
  CLERK_WEBHOOK_SIGNING_SECRET: optional(z.string().startsWith('whsec_', 'começa com whsec_')),
  ...
  BILLING_MODE: z.preprocess((value) => (value === '' ? undefined : value), z.enum(['asaas', 'off']).default('asaas')),
  ASAAS_ENV: z.preprocess((value) => (value === '' ? undefined : value), z.enum(['sandbox', 'production']).default('sandbox')),
...
export function serverEnv(): ServerEnv {
  ...
  if (!result.success) {
    ...
    throw new Error(`Variáveis de ambiente inválidas — ${problems.join('; ')}. Confira o .env.example.`);
  }
  parsed = result.data;
  return parsed;
}
