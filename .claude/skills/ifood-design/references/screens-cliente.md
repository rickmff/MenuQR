...
5. **Identidade** (`StoreIdentity`, client), linha `flex items-start gap-3`:
   ...
   - coluna `pt-2`: nome `font-display text-h4 font-bold leading-tight text-gray-900 truncate lg:text-h3`; abaixo o **`StoreStatus`** (`mt-1`, `aria-live="polite"`): ponto `size-2 rounded-full` (`bg-positive` / `bg-gray-400`) + `text-body1 font-semibold` — aberto, **"Aberto até 23:00"** em `text-positive`; fechado, o `describeNextOpening` ("Abre hoje às 18:00", "Abre amanhã às …") em `text-gray-700`. Pedido mínimo numa **linha própria** `text-body2 text-gray-600` ("Mínimo para entrega R$ 25,00", sempre que a entrega está ligada — não depende do modo escolhido, senão a linha some com Retirada e o Entrega | Retirada pula) — ao lado do status ela quebrava. Antes de hidratar, `Skeleton` `h-4 w-40`: o status depende do relógio e a página é ISR. Recalcula a cada minuto, no fuso da loja;
...
2. **Barra compacta com o nome**: `fixed inset-x-0 top-0 z-40 border-b border-gray-200 bg-white pt-safe` no celular; no desktop `lg:sticky lg:-mb-14` no topo da coluna que rola do painel, com o "‹" (`lg:absolute lg:top-1.5`) sobre ela, linha `h-14 px-16` com o título `text-body1 font-semibold text-gray-900 truncate`. Entra com fade e 4px (150 ms) quando a foto sai (sentinela + `IntersectionObserver`), `aria-hidden` + `inert` enquanto invisível. Sem foto ela já nasce visível, e um espaçador `h-16` fica no lugar da foto (também no desktop: o "‹" cobria o título).
...
| Cardápio despublicado (ou ainda em rascunho)          | `StoreUnavailable reason="unpublished"` (desde 2026-09-25): o mesmo aviso sem casca, com o nome da loja — "Cardápio fora do ar no momento" —, `noIndex` e sem CTA de venda; também em `/item/…`. Slug que não existe continua no 404 global. O demo mostra o mesmo aviso para o restaurante deste navegador em rascunho ou despublicado (`DemoStoreLayout`); slug desconhecido continua no `NotFound` do demo, com "Ver exemplo". Link do demo que já traz o cardápio no endereço (`#c=`) abre mesmo depois de despublicar: o painel do demo avisa isso no rascunho, na confirmação de despublicar e na prévia |
