...
- **Quick-add** (funcionalidade mantida, no padrão da página de item do iFood): um `Plus` de 24px numa coluna própria depois da foto, colado na borda direita da lista — sem círculo, sem fundo e sem sombra. Alvo de toque de 44px (`grid size-11 place-items-center rounded-full`), puxado para fora com `-mr-2.5` para o glifo encostar na margem.
- **A coluna tem largura fixa**, `w-(--menu-action-width)` (5,625rem = a medida do `Stepper size="sm"`: 32 + 24 + 32 + 2 de borda), com `justify-end`. Ela existe em toda linha, com ou sem foto, e em todos os estados. Sem essa largura a foto escorrega ~46px na linha em que o "+" virou "− 1 +", e a lista perde a régua vertical — foi exatamente o que aconteceu quando o stepper inline entrou. Ao mexer no `Stepper`, recalcule o token em `globals.css`.
