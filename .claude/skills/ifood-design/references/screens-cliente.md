...
- Foto hero `aspect-4/3` de borda a borda, sem raio. Sobre ela, `IconButton variant="raised"` de voltar (um `Link` para o cardápio) e de compartilhar. Sem foto: app bar comum com voltar.
- Nome `text-h6 font-bold` (o `h1`), descrição `text-body2 text-gray-600`, uma linha `text-caption text-gray-600` com "Serve 1 pessoa • 720 kcal" e "Contém: glúten, leite", `Price` em `text-body1 font-semibold` e a nota de pedido mínimo em `text-caption`.
- **Grupos de opções**, separados por `Divider thick`. Cabeçalho sobre `bg-gray-50 px-4 py-3`: à esquerda o nome `text-body1 font-semibold` e o helper `text-caption text-gray-600` ("Escolha 1 opção", "Escolha até 2 opções", "Escolha quantas quiser"); à direita `Tag tone="dark"` "OBRIGATÓRIO", que vira um `Check` verde quando o grupo é satisfeito. Opções em `RadioRow`/`CheckboxRow` com "+ R$ 3,00" à direita; ao atingir o máximo, as não marcadas desabilitam.
- Grupo obrigatório **não vem pré-selecionado**: a escolha é da pessoa, e o botão só libera quando ela escolhe.
- "Algum comentário?" — `TextArea` com `maxLength={140}`, contador e placeholder "Ex: tirar a cebola, maionese à parte, etc.".
- **Barra inferior fixa**: `Stepper` (mínimo 1) + `Button fullWidth` "Adicionar" com o total à direita. Desabilitado, com o helper "Escolha as opções obrigatórias", até todos os obrigatórios estarem satisfeitos.
