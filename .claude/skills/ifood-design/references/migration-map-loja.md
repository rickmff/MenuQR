...
- **Manter**: o `<li>`; o `Link` cobrindo a linha (linhas 28 e 40-44); `aria-disabled={!item.available}`; `priority`; `DishImage` com `alt={item.imageAlt || item.name}` e `sizes="96px"`; `formatPrice`; a decisão `canQuickAdd = item.available && !item.hasRequiredOptions` (linha 29); o botão de quick-add como irmão do `Link`, fora dele; a largura fixa da coluna de ação (`w-(--menu-action-width)`), sem a qual a foto escorrega quando o "+" vira stepper; o rótulo "Adicionar {nome} à sacola" no `aria-label`.
