...
- `FRAMES`: `outlined` `rounded-sm border border-gray-300 bg-white` (padrão); `plain` nada; **`soft`** `rounded-full bg-gray-100`; **`floating`** `rounded-full bg-white shadow-medium` (2026-09-24). `INK`: `outlined`/`plain` pintam os glifos de `text-primary`; `soft`/`floating`, de `text-gray-900`, como na referência.
...
- Onde: prato `lg soft` (`min=1`, centrado antes do CTA); linha da sacola `sm soft` com `onRemove`; adicional com quantidade `sm soft` (`StepperRow`, `min=0`); pílula do cardápio `sm floating` com `onRemove` (`QuickPill`). O painel não usa mais.
...
- **`ItemCard`** (`memo`): `li.relative` › `Link` (`press -mx-4 flex gap-4 px-4 py-4 active:bg-gray-50 lg:rounded-md lg:hover:bg-gray-50`, `transitionTypes={['nav-forward']}` fora da prévia; com `storeSlug`, o clique grava a posição) com nome `h3.line-clamp-2.text-subtitle.font-semibold.text-gray-700`, descrição `line-clamp-1 text-body2 text-gray-600`, preço `text-body1 font-bold tabular-nums text-gray-900` + a primeira tag (`Tag ink`), e a foto `DishImage size-24 rounded-lg` (`sizes="96px"`, `fade`). A ação é **irmã** do link (interativo dentro de link navegaria), `absolute`: com foto em `right-2 top-18`, sobre o canto de baixo da foto, num círculo `bg-white text-gray-900 shadow-medium hover:shadow-high`; sem foto em `right-0 top-1/2 -translate-y-1/2`, círculo `bg-gray-100`, com o texto em `pr-12`. Quatro estados: quick-add = `<button>` 32 `hit-44` "Adicionar {nome} à sacola" + `CountBadge`; com obrigatório = o mesmo círculo como `Link` "Escolher as opções de {nome}"; na sacola = `QuickPill` (`Stepper sm floating` com `onRemove`, `role="group" aria-label="{nome} na sacola"`, `origin-right animate-pill-reveal` só depois da carga); indisponível = sem ação, `opacity-60`, foto `grayscale` com o selo "Indisponível" `bg-gray-800/85` (sem foto, `Tag ink`).
